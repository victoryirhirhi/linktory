...
bot.action("DASH_STATS", async (ctx) => {
  ...
  try {
    ...
    if (rows.length === 0) return ctx.reply("⚠️ User not found.", dashboardKeyboard());
...
bot.on("text", async (ctx) => {
  ...
  try {
    ...
    if (state?.action === "ADD_LINK") {
      if (!/^https?:\/\//.test(text)) return ctx.reply("⚠️ Invalid link.", dashboardKeyboard());
    ...
    if (!state && /^https?:\/\//.test(text)) {
      ...
      return ctx.reply("🔗 Detected a link! What do you want to do?", Markup.inlineKeyboard([
